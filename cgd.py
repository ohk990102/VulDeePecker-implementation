...
cwe_119_keywords = tuple(
    map(lambda v: v.strip(), open("./cwe-119.txt", "r").read().split(","))
)
cwe_399_keywords = tuple(
    map(lambda v: v.strip(), open("./cwe-399.txt", "r").read().split(","))
)
...
class CGDDataset(data.Dataset):
    def __init__(self, file: str, vector_length: int):
        ...
        self.keywords = (
            cpp_keywords + extra_keywords + cwe_119_keywords + cwe_399_keywords
        )
