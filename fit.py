    ...
    def cross_validation(self, dataset):
        ...
        for fold, (train_index, test_index) in enumerate(kfold.split(dataset)):
            ...
            train_loader = torch.utils.data.DataLoader(
                train_fold, batch_size=self.config.batch_size, shuffle=False
            )
            test_loader = torch.utils.data.DataLoader(
                test_fold, batch_size=self.config.batch_size, shuffle=False
            )

            epochs = self.config.num_epochs

            for epoch in range(epochs):
                train_summary_loss, train_total_score = self.train_one_epoch(
                    train_loader
                )
                val_summary_loss, val_total_score = self.validation(test_loader)
                print()
                print(
                    f"Epoch {epoch+1}, Train F1: {train_total_score.f1:.5f}, Validation F1: {val_total_score.f1:.5f}, "
